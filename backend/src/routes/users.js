...
router.get('/:id', auth, (req, res) => {
  const user = db.prepare(
    'SELECT id,name,email,role,profession,company,profile_picture,looking_for,public_key,created_at FROM users WHERE id=?'
  ).get(req.params.id);
...
router.put('/me', auth, upload.single('profile_picture'), (req, res) => {
  const { name, profession, company, looking_for, public_key } = req.body;
  ...
  const updated = db.prepare(
    'SELECT id,name,email,role,profession,company,profile_picture,looking_for,public_key FROM users WHERE id=?'
  ).get(req.user.id);
  res.json(updated);
});
